...
/**Got help on from a classmate and tutor on how to do this.
 * @author Gurjus Singh. **/
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        while (_input.nextIf(",")) {
            String nextColName = columnName();
            colnames.add(nextColName);
        }
        ...
        if (_input.nextIf(",")) {
            ...
            result = table.select(secondTable, colnames, conditions);


        } else {
            conditions = conditionClause(table);
            result = table.select(colnames, conditions);
        }
    ...
    Condition condition(Table... tables) {
        ...
        String lit = null;
        Column c1 = new Column(col, tables);
        Column c2 = null;
        if (_input.nextIs(Tokenizer.LITERAL)) {
            lit = literal();
            return new Condition(c1, relation, lit);
        } else {
            lit = columnName();
            c2 = new Column(lit, tables);
