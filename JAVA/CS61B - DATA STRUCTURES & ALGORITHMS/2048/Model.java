    ...
    boolean tilt(Side side) {
        ...
        for (int col = 0; col <= size() - 1; col++) {
            for (int row = size() - 1; row >= 0; row--) {
                int i = row;
                while (i > 0) {
                    Tile next = vtile(col, row, side);
                    current = vtile(col, i - 1, side);
                    if (next == null && current != null) {
                        setVtile(col, row, side, current);
                        changed = true;


                    } else if (next != null && current
                            !=
                            null && current.value() == next.value()
                            && !merged) {
                        setVtile(col, row, side, current);
                        changed = true;
                        merged = true;
...
/**
 * Basically gettilt method for gameover*
 * returns true
 * SIDE PARAM.
 */

    boolean tiltgameover(Side side) {
        boolean merged = false;
        ...
        for (int col = 0; col <= size() - 1; col++) {
            for (int row = size() - 1; row >= 0; row--) {
                int i = row;
                while (i > 0) {
                    Tile current = vtile(col, row, side);
                    Tile next = vtile(col, i - 1, side);
    ...
    private void checkGameOver() {
        ...
        if (!tiltgameover(Side.EAST)
                && !tiltgameover(Side.NORTH)
                &&
                !tiltgameover(Side.WEST)
                &&
                !tiltgameover(Side.SOUTH)) {
            _gameOver = true;

        }

        if (gameOver() && _maxScore < _score) {
            _maxScore = _score;
        }
    }
