...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    lats= [restaurant_location(x)[0] for x in cluster]
    longs = [restaurant_location(x)[1] for x in cluster]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(elements) for elements in clusters]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    X =  sum([(x-mean(xs))**2 for x in xs])
    LST = [(xs[k]-mean(xs))*(ys[k]-mean(ys)) for k in range(0, len(ys))]
    XY =  sum(LST)
    Y =  sum([(y-mean(ys))**2 for y in ys])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    X= [find_predictor(user,reviewed,fn) for fn in feature_fns]


    return max (X , key = lambda x:x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            predictions[restaurant_name(restaurant)]=user_rating(user, restaurant_name(restaurant)) 
        else:
            predictions[restaurant_name(restaurant)] = predictor(restaurant)
